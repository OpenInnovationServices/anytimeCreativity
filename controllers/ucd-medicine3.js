...
exports.index = (req, res, next) => {
    res.render('ucd-medicine3/index', {
...
exports.dashboard = (req, res, next) => {
    MongoClient.connect(keys.mongodb.dbURI, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, db) {
        ...
        dbo.collection("challengeDataCollection").find({ "code": "1005" }).toArray(function(err, result) {
            if (err) res.status(400).json("Error Connecting DB");
            res.status(200).render('ucd-medicine3/dashboard', {
...
exports.vote1 = (req, res, next) => {
    MongoClient.connect(keys.mongodb.dbURI, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, db) {
        ...
        dbo.collection("challengeDataCollection").find({ "code": "1005" }).toArray(function(err, result) {
            if (err) res.status(400).json("Error Connecting DB");
            res.render('ucd-medicine3/vote1', {
...
exports.vote2 = (req, res, next) => {
    MongoClient.connect(keys.mongodb.dbURI, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, db) {
        ...
        dbo.collection("challengeDataCollection").find({ "code": "1005" }).toArray(function(err, result) {
            if (err) res.status(400).json("Error Connecting DB");
            res.render('ucd-medicine3/vote2', {
                pageTitle: 'UCD Medicine Vote',
                data: result
            });
            db.close();
...
exports.voteDashboard = (req, res, next) => {
    MongoClient.connect(keys.mongodb.dbURI, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, db) {
        ...
        dbo.collection("challengeDataCollection").find({ "code": "1005" }).toArray(function(err, result) {
            if (err) res.status(400).json("Error Connecting DB");
            res.render('ucd-medicine3/voting-dashboard', {
...
exports.idea = (req, res, next) => {
    MongoClient.connect(keys.mongodb.dbURI, { useUnifiedTopology: true, useNewUrlParser: true }, function(err, db) {
        if (err) throw err;
        var dbo = db.db(keys.mongodb.dbName);
        dbo.collection("challengeDataIdeaCollection").find({ "code": "1005" }).toArray(function(err, result) {
            if (err) res.status(400).json("Error Connecting DB");
            res.render('ucd-medicine3/vote-idea', {
...
exports.result = async (req, res, next) => {
    ...
    try{
        ...
        const result1 = await collection.find({ "code": "1005" }).sort( { count: -1 } ).toArray()
        const result2 = await ideaCollection.find({ "code": "1005" }).toArray()

        res.render('ucd-medicine3/vote-result', {
            pageTitle: 'UCD Medicine Idea',
...
exports.resultDashboard = async (req, res, next) => {
    ...
    try{
        ...
        res.render('ucd-medicine3/vote-result-dashboard', {
            pageTitle: 'UCD Medicine Idea',
            data: { result1, result2 }
        });
